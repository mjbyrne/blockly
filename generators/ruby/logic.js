...
Blockly.Ruby['controls_if'] = function(block) {
  ...
  do {
    ...
    code += (n == 0 ? 'if ' : 'elsif ' ) + conditionCode + ':\n' + branchCode;
  ...
  if (block.getInput('ELSE')) {
    ...
    code += 'else\n' + branchCode;
    //potentially need to add indent for next line
  }
  return code;
...
Blockly.Ruby['logic_compare'] = function(block) {
  ...
  //order has to do with operator precedence 
  //have to create operator order values in ruby master file under reserved words
  //http://ruby-doc.org/core-2.4.1/doc/syntax/precedence_rdoc.html
  var order = (operator == '==' || operator == '!=') ?
      Blockly.Ruby.ORDER_EQUALITY : Blockly.Ruby.ORDER_RELATIONAL;
...
Blockly.Ruby['logic_operation'] = function(block) {
  // Operations 'and', 'or'.
  //in ruby && and || have higher precedence than and and or, which to use?
...
Blockly.Ruby['logic_ternary'] = function(block) {
  // Ternary operator.
  // have to check nil data type
  // nil is a method to by called not a type
